...
class MiniDoubleMobile(torch.nn.Module):
    ...
    def forward(self, x, net_idx=None):
        if net_idx is None:
            cls_, adapt_cls = self._inner_call(x, 0)
            energy_, adapt_energy = self._inner_call(x, 1)
            if not self.training:
                return cls_, energy_
            return cls_, energy_, adapt_cls, adapt_energy
        elif net_idx == 0:
            cls_, adapt_cls = self._inner_call(x, net_idx)
            return cls_, None, adapt_cls, None
        elif net_idx == 1:
            energy_, adapt_energy = self._inner_call(x, net_idx)
            return None, energy_, None, adapt_energy

    def _inner_call(self, x, index):
        ext, cls, extra_cls = ((self.ext1, self.cls1, self.lin1_extra), (self.ext2, self.cls2, self.lin2_extra))[index]
        x = ext(x)
        if index:
            self.last2 = x
        else:
